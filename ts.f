...
! Inicia: Implante Interface Multi Fortran
...
c       Ah, lo hago de esta forma, ya que esta informacion, solamente
c       sera para tener el nombre de la dinamica y DatosGrlsDinamica
c       seran para tener mas informacion aun, lo interesante sera probar
c       abriendo y cerrando el archivo...
c       no.. no le veo uso.. al ckout, lo eliminare en esta version, si
c       si es que tiene utilidad ya se requerira luego.
...
* restart the dynamics
     ...
     & Gradx,Grady,Gradz, x0,y0,z0,sK,refPE,ibTime1,ibTime2,ibTime3)
                                     !
      write (77,*) "restarting the dynamics from:"
      write (77,"(a,i6,1x,f9.2)") "step , time", ibegin, trest
                                     !
...
* ----------------------------------------------
     ...
     &    xnew,ynew,znew, vxnew,vynew,vznew, rfac,vfac)
      ...
      write (77,*) "--------------------"
      write (77,"(1x,a,i6,1x,f9.2)") "step & time", -1, tt
...
c     if (imethod.eq.2) call system ("./script.nwchem")
! Inicia: Implante Interface Multi Fortran
      if (imethod.eq.2) call system ("sh "//trim(NwchemScript))
! Termina: Implante Interface Multi Fortran          !
      call sleep (02)                   !
...
* 1 nano-metre= 10 Angst
      ...
      write (77,11) "  energies [Hartree] t=  ", tt
...
* ------------------------------------------
     ...
     &   xnew,ynew,znew, vxnew,vynew,vznew, dt)
      ...
      write (77,*) "--------------------"
      write (77,"(1x,a,i6,1x,f9.2)") "step & time", 0, tt
...
* ----------------------
      ...
      kj= kj + 1                         !
                                         !
      write (77,*) "--------------------" !
      write (77,"(1x,a,i6,1x,f9.2)") "step & time", k, tt
                                         !
                                         !
...
c     call distances (tt,npar, x,y,z)    !
      ...
      write (77,"(/,a)") "ionizing radiation: bond stochastically 
     & broken"
                                         !
...
! Termina: Implante Interface Multi Fortran          !                                   !
     ...
     &  Gradx,Grady,Gradz,Epot,refPE)        !
      ...
      wLength= (455.619348)/(ene2-ene1)  ! in Angst
                                         !
      write (77,"(/,a,f9.5)") "ionization energy [eV]", radEne
      write (77,"(a,e14.5)")  "energy wavelength [Angst]", wLength
      call makeMovie (k,npar, atsymbol,x,y,z)
      end if                             !
                                         !
                                 ! -------------
     ...
     &    (k.eq.ibTime3+iAft)) then      !
      write (77,"(/,a)") "charge uptake from environment"
                                         !
...
* create the speeds histogram and the analytic Maxwell speed distrib. funct.
     ...
     &,avSpd
      ...
      write (77, *)
      write (77, *), "the speed histogram of the cage particles 
     & was created"
      write (77, *), "the analytic Maxwell speed distrib. funct. i
     & was created"
...
* print energies in terms of t
     ...
     &  Pdyn [GPa]"
      ...
      do kk= 1, nsteps, 1 ! <---
       ...
       sum2= sum2 + tCage(kk)**2!
                                !
       write (77,10) tt, enekin(kk),enepot(kk),
     &      tCage(kk),tConf(kk),press(kk)
      ...
      write (77, *)
      write (77,*) "number of computed time steps= ", kj
      write (77,*)
      ...
      write (77,22) "charge grps", ngrp1,ngrp2,ngrp3
...
c     write (77,"(a,1x,f6.3)") "distance from cage for cooling down",
c    &               thrDist
c     write (77,"(a,1x,i4)") "num of atoms close to walls", icont
...
!     include 'parametros'
      ...
      external kparse
! Inicio: Implante Interface Multi Fortran
        CHARACTER (LEN = 400) NombreDinamica
        CHARACTER (LEN = 400) RutaArchivosEntrada
        CHARACTER (LEN = 400) RutaArchivosSalida
        CHARACTER (LEN = 400) NombreOllaOut
...
* reduce time steps for large forces
      ...
      write (77,*)  "largest positive forces in x,y,z"
      ...
      if (dt.ne.dtIni) write (77,"(a,f9.2)") "the time step is", dt
      write (77,*)
...
* ------------------------------
      ...
      if (xenergy.eq.0.0) then
        write (77, *), "TERACHEM: got NO energy"
...
* -------------------------------------------
      ...
      write (77,"(a)") "1st broken bond"
      write (77,"(2(2x,a2,i4),a,f8.3)") atsymbol(k), k,
     &       atsymbol(j1), j1, " with distance", smalla
      ...
      write (77,"(a,i4,1x,a,f9.3)") "increment for atom ", k,
     &    atsymbol(k), dist2

      dist3= sqrt((x(j1)-d)**2 + (y(j1)-e)**2 + (z(j1)-f)**2)
      write (77,"(a,i4,1x,a,f9.3)") "increment for atom ", j1,
     &    atsymbol(j1), dist3

      write (77,"(a,f9.3)") "total increment", dist2+dist3
      write (77,"(a,f9.3)") "increase factor", dist1/smalla
...
* -------------------------------------------
      ...
      write (77,"(a)") "2nd broken bond"
      write (77,"(2(2x,a2,i4),a,f8.3)") atsymbol(j2), j2,
     &       atsymbol(j3), j3, " with distance", small
      ...
      write (77,"(a,i4,1x,a,f9.3)") "increment for atom ", j2,
     &    atsymbol(j2), dist2

      dist3= sqrt((x(j3)-d)**2 + (y(j3)-e)**2 + (z(j3)-f)**2)
      write (77,"(a,i4,1x,a,f9.3)") "increment for atom ", j3,
     &    atsymbol(j3), dist3

      write (77,"(a,f9.3)") "total increment", dist2+dist3
      write (77,"(a,f9.3)") "increase factor", dist1/small

      write (77,"(a,i3,a,i3,f9.3)") "distance between atoms ", k,
     &     " and ", j2, smallb 
...
c     write (77,"(a,3f9.3)") atsymbol(j1), x(j1), y(j1), z(j1)
...
! Termina: Implante Interface Multi Fortran
      ...
      if (imethod.eq.1) then
      write (77, *), "Module: Pressure Cooker (TeraChem version)"
      end if

      if (imethod.eq.2) then
      write (77, *), "Module: Pressure Cooker (NWChem version)"
      end if

      write (77, *), "Ruben Santamaria:   rso@fisica.unam.mx"
...
* *********************
      ...
      write (77,*) "figure from: http://www.asciiworld.com"
      write (77,*)
      write (77,*) "          _,.-------.,_          "
      write (77,*) "      ,;~'             '~;,      "
      write (77,*) "    ,;                     ;,    "
      ...
      write (77,*) " ,;                           ;, "
      write (77,*) " ; ;      .           .      ; ; "
      write (77,*) " | ;   ______       ______   ; | "

c     write (77,*) " |  `/~"     ~" . "~     "~\'  | "
      write (77,*) " |  `/~","     ~", char(34), "  .  ", char(34),
     &     "~          |"
...
c     write (77,*) "  .~  (__,.--" .^. "--.,__)  ~.  "
      write (77,*) "  .~  (__,.--", char(34), " .^. ", char(34),
     & "--.,__)  ~."

      write (77,*) "  |     ---;' / | \ ';---     |  "
      write (77,*) "   \__.       \/^\/       .__/   "
      write (77,*) "    V| \                 / |V    "
      write (77,*) "     | |T~\___!___!___/~T| |     "
      write (77,*) "     | |`IIII_I_I_I_IIII'| |     "
      write (77,*) "     |  \,III I I I III,/  |     "
      write (77,*) "      \   `~~~~~~~~~~'    /      "
      write (77,*) "        \   .       .   /        "
      write (77,*) "          \.    ^    ./          "
      write (77,*) "            ^~~~^~~~^            "
      write (77,*)
      write (77,*) "         THIS IS THE END"
      write (77,*) "* ----------------------------- *"
      write (77,*)
...
* ------------------------------
      ...
      write (77,12) time, sum1,sum2,sum3, sum1+sum2+sum3
...
c     do k= 1, 3 ! <---------
c                            !
c     iBreakTime(k)= 0       ! default value
c                            !
c  10 eta= zbqlu01()         ! zbqlu01 is in [0,1]
c     n1= int(eta*nsteps)    ! n1 is in [0,nsteps]
c                            !
c     if ((n1.lt.100).or.(n1.gt.nsteps-100)) goto 10
c     iBreakTime(k)= n1      ! this break time is taken
c                            !
c     end do ! <-------------
...
* consecutive numbers to give chance to the charge uptake
      ...
      write (77,"(/,1x,a,3i7)") "rnd times to break bonds",
...
* ---------------------------
      ...
      do i= 1, ncage ! <-----
                             !
      if (i.eq.1) write (77,*) "spring constants were made different"
                             !
...
c     write(ij,*) "echo"
     ...
     & mb"
c      write(ij,*) "permanent_dir /home/rso/work/He110/permanent"
c      write(ij,*) "scratch_dir   /home/rso/work/He110/scratch

! Inicia: Implante Interface Multi Fortran
      write(ij,*) "permanent_dir "// trim(RutaPermanent)
      write(ij,*) "scratch_dir " //trim(RutaScratch)
! Termina: Implante Interface Multi Fotran

      write(ij,*)

      if (ilabel.eq.4) write (77, *), "charge changed from -1 to 0"
...
* --------------------
      ...
      if (ilabel.eq.4) write (77, *), "multiplicity changed from 2 to 1"
      ...
      if ((ilabel.eq.2).or.(ilabel.eq.5)) then
c      write(ij,*)
c     &  " vectors input /home/rso/work/permanent/molec.movecs"

! Inicia: Implante Interface Multi Fortran
       write(ij,*)
     &  " vectors input "//trim(RutaPermanent)
     &//"/molec.movecs"
! Termina: Implante Interface Multi Fortran
      end if
...
* -------------------------
      ...
      if (ilabel.eq.3) then
        write (77, *), "the ab-initio input file may be"
        write (77, *), "changed in module inptTera"
        ij= 6
      else
        open (14,file="terachem.inp",status="unknown")
        write (77,"(/,a,i2)") "WF type", ilabel
        ij= 14
      end if
...
c     open (unit=13, file= 'olla.inp', status= 'unknown')
...
* ------------------------------
      ...
      if (irestDyn.eq.1) write (77,14) 
     & "dynamics starts for the 1st time"
      if (irestDyn.eq.2) write (77,14) "restarting the dynamics"

      if (itest.eq.1)   write (77,14) "running a test"

      if (itest.eq.2) then ! <---
                                 !
      write (77,14) "running ab-initio computation"
                                 !
      if (imethod.eq.1) write (77,14) "using TeraChem"
      if (imethod.eq.2) write (77,14) "using NWChem"
                                 !
      if (iwavFunct.eq.1) write (77,14)
     &  "creating the wave function from scratch"
      ...
      write (77,*)
      write (77,*) "N cage atoms > N conf atoms"
      write (77,*) "!!!! be a smart guy !!!!"
      write (77,*)
      stop
      end if
      ...
      write (77,14) "ncage     ",      ncage
      write (77,18) "charge for group of atoms", ngrp1,ngrp2,ngrp3
...
* ------------------------
      ...
      if (ilabel.eq.2) then ! <---
                                  !
      write (77,"(a)") "reading stored variables"
                                  !
      read(15,*) name, ibegin     !
      read(15,*) name, time       !
      read(15,*) name, dt         !
...
! Inicia: Implante Interface Multi Fortran       
      ...
      parameter (maxchr=100, maxtok=10)
        ...
        CHARACTER (LEN = 400) NwchemInp
        CHARACTER (LEN = 400) NwchemScript
        CHARACTER (LEN = 400) RutaMPI
        INTEGER NumeroProcesadores
        CHARACTER (LEN = 400) CharNumProcesadores
        CHARACTER (LEN = 400) RutaNWChem 

        call DatosGrlsDinamica(NombreDinamica, 
     & RutaArchivosEntrada, RutaArchivosSalida)

        call RutinaDatosNwchem2(RutaMPI, 
     & RutaNWChem, NumeroProcesadores) 
...
* -------------------
      ...
      if (imethod.eq.1) then ! <-----------
                                           !
      write (77, *), "creating the script to run terachem"
...
! Inicia: Implante Interface Multi Fortran
      ...
      if (NumeroProcesadores.eq.0) then
        
        write(14,*) trim(RutaNWChem)//" "//trim(NwchemInp)
     &//">"//trim(NwchemOut)

      else if (NumeroProcesadores.ne.0) then
        
        write(CharNumProcesadores,'(i2)') NumeroProcesadores
        write(14,*) trim(RutaMPI)//" -np "//trim(CharNumProcesadores)
     &//" "//trim(RutaNWChem)//" "
     &//trim(NwchemInp)//">"//trim(NwchemOut)
...
! Termina: Implante Interface Multi Fortarn     
...
* **************************
     ...
     & // '.out'
      ...
      write (77,*) "-------------------------"
      write (77,*) "physical variables & their unis"
      write (77,*) "-------------------------"
      write (77,*) "[position]= Angst              [mass]= Kg"
      write (77,*) "[speeds]=   Angst/fs           [time]= fs"
      write (77,*) "[acceleration]= Angst/fs^2     [Force]= Newton"
      write (77,*) "                               [Energy]= Joules"
      write (77,*)

      write (77,*) "1 Angst= 1x10^{-10} mt"
      write (77,*) "1 mt=    1x10^{+10} Angst"
      write (77,*) "1 Bohr= 0.529178 Angst= 0.529178 x10^{-10} mt"
      write (77,*) "1 amu=   1.6605402 x10^{-27} Kg"
      write (77,*) "1 Hartree= 4.35988 x10^{-18} J"
      write (77,*) "1 Hartree= 27.211396132 eV"
      write (77,*) "1 fs= 1x10^{-15} s"
      write (77,*) "1 atmosphere= 101325.0 pascals"
      write (77,*) "1 pascal= 1/101325.0 atmosphere"
      write (77,*)

      write (77,*) "Newton= Kg *mt/ s^2"
      write (77,*) "Joule= Newton * mt= Kg *(mt/s)^2"
      write (77,*)

      write (77,*) "kB= 1.3806504 x10^{-23}    in Joule/Kelvin"
      write (77,*) "pi= 3.1415926535898"
      write (77,*) "c= 2.99792458 x10^8 m/s= 2.99792458 x10^{10} cm/s",
     &          " speed of light"
      write (77,*) "mole= 6.02214179 x10^{23}"
      write (77,*) "R= 8.3144621 J/(mol*Kelvin)   universal constant",
     &        " of gases"
      write (77,*) "Planck constant= hbar=6.62606957x10^{-34} 
     & (kg*m^2)/s"
      write (77,*)
      write (77,*) "--------------------------------------"
      write (77,*) "units of positon= acceleration * dt^2"
      write (77,*) "--------------------------------------"
      write (77,*) "[atom mass]= atomass x amu"
      write (77,*) "[Force]= [dE/dx]= Hartree/Bohr"
      write (77,*) "[dt]= fs"
      write (77,*)

      write (77,*) "rx= ax * dt^2 /2"
      write (77,*) " = (Fx/m) * dt^2 /2"
      write (77,*) " = -(1/m) (dE/dx) *dt^2 /2"
      write (77,*) " = -(1/m) (Gradx) *dt^2 /2 [(1/amu)*Hartree/Bohr",
     &            " *fs^2]"
      write (77,*)

      write (77,*) "[(1/amu) *Hartree/Bohr *fs^2]"
      write (77,*) "= 1/(1.6605402 x10^{-27} Kg) *"
      write (77,*) "    4.35988 x10^{-18} J/ 0.529178 x10^{-10} mt",
     &             " *1x10^{-30} s^2"
      write (77,*) "= 4.35988 /(1.6605402 * 0.529178)x10^{+27-18+10-30}"
     &           , " J/(Kg*mt)*s^2"
      write (77,*) "= 4.9616184 x10^{-11} m"
      write (77,*)

      write (77,*) "transformation factor for positions derived from",
     &           " the force"
      write (77,*)

      write (77,*) "faccl= 0.49616184    dimensionless"
      write (77,*)

      write (77,*) "faccl*Angst= (1/amu)*(Hartree/Bohr)*(fs^2)"
      write (77,*)

      write (77,*) "----------------------------------"
      write (77,*) "units of speed= acceleration * dt"
      write (77,*) "----------------------------------"
      write (77,*) "from the result for positions we have:"
      write (77,*) "fvel= 0.49616184"
      write (77,*) "fvel *(Angst/fs)= (1/amu) *(Hartree/Bohr) *fs"
      write (77,*)

      write (77,*) "------------------------"
      write (77,*) "units of kinetic energy"
      write (77,*) "------------------------"
      write (77,*) "[Ek]= [mass *vx(i)^2]= amu *(Angst/fs)^2"
      write (77,*)

      write (77,*) "1 amu = 1.6605402 x10^{-27} Kg"
      write (77,*) "1 Angst= 1x10^{-10} mt"
      write (77,*) "1 fsec= 1x10^{-15} sec"
      write (77,*)

      write (77,*) "[Ek]= [1.6605402x10^{-27} Kg] [1x10^{-10} mt]^2/",
     &            " [1x10^{-15} sec]^2"
      write (77,*)
      ...
      write (77,*) "transformation factor between units of kinetic",
     &            " energy"

      write (77,*) "facEkin= 1.6605402d-17"
      write (77,*) "facEkin * Joule= amu *(Angst/fs)^2"
      write (77,*)

      write (77,*) "-----------------------------"
      write (77,*) "units of pressure= force/area"
      write (77,*) "-----------------------------"
      write (77,*) "radial force= F . R/|R|= -(dE/dx,dE/dy,dE/dz) .",
     &            " R/|R| [Hartree/Bohr]"
      write (77,*)

      write (77,*) "[Hartree/Bohr]= 4.35988 x10^{-18} J/",
     &            " 0.529178x10^{-10} mt"
      write (77,*) "= (4.35988/ 0.529178) x10^{-8} [J/m=Newton]"
      write (77,*) "= 8.2389668504738d-08 [N]"
      write (77,*)

      write (77,*) "area [Angst^2]= 1x10^{-20} mt^2"
      write (77,*)

      write (77,*) "pressure= radial force/area= 8.2389668504738d+12",
     &            " [N/m^2=Pa]"
      write (77,*)

      write (77,*) "facPres= 8.2389668504738d+12"
      write (77,*) "facPres * Pa= (Hartree/Bohr)/(Angst^2)"
      write (77,*)

      write (77,*) "Joule/Angst^3= Joule/[1x10^{-30} mt^3]"
      write (77,*) "             = 1x10^{30} Newton/mt^2"
      write (77,*) "             = 1x10^{30} Pascal"
      write (77,*)

      write (77,*) "facPres2= 1.x10^{30}"
      write (77,*) "facPres2 * Pa= Joule/Angst^3"
      write (77,*)

      write (77,*) "-------------"
      write (77,*) "units of kT"
      write (77,*) "-------------"
      write (77,*) "1 Joule= 2.29371248 x 10^{17}   Hartree"
      write (77,*) "kB= 1.3806504 x 10^{-23}       Joule/Kelvin"
      write (77,*) "  = 1.3806504x10^{-23} * 2.29371248x10^{17}",
     &            " Hartree/Kelvin"
      write (77,*) "  = 3.16681505 x 10^{-6}     Hartree/Kelvin"
      write (77,*)

      write (77,*) "[Temp]= Kelvin"
      write (77,*) "[kB*Temp]= Joule"
      write (77,*)

      write (77,*) "1 amu= 1.6605402 x10^{-27} Kg"
      write (77,*) "m/s= 1x10^{+10)/1x10^{+15} Angst/fs= 1x10^{-5}",
     &            " Angst/fs"
      write (77,*)

      write (77,*) "[kB*Temp/amu]= J/ (1.6605402x10^{-27} Kg)"
      write (77,*) "  = (m/s)^2/ (1.6605402x10^{-27})"
      write (77,*) "  = (Angst/fs)^2 * 1x10^{-10})/ 
     & (1.6605402x10^{-27})"
      write (77,*) "  = 1x10^{+17}/1.6605402 (Angst/fs)^2"
      write (77,*)

      write (77,*) "facKT= 1.0d17 /1.6605402"
      write (77,*) "facKT *(Angst/fs)^2= (Joule/Kelvin)*(Kelvin/amu)"
      write (77,*)

      write (77,*) "-------------------------"
      write (77,*) "units of spring constant K"
      write (77,*) "-------------------------"
      write (77,*) "[spring K]= Newton/cm"
      write (77,*) "1 amu= 1.6605402 x10^{-27} Kg"
      write (77,*)

      write (77,*) "Newton/cm= Kg*mt/(s^2*cm)"
      write (77,*) "         = Kg*100cm/ (1x10^{30} fs^2 *cm)"
      write (77,*) "         = Kg/(1x10^{28} fs^2)"
      write (77,*) "         = amu/(1.6605402 x10^{-27} *1x10^{28} 
     & fs^2)"
      write (77,*) "         = (amu/fs^2) *1.0/16.605402"
      write (77,*)

      write (77,*) "fspring= 1.0/ 16.605402"
      write (77,*) "Newton/cm = fspring *(amu/fs^2)"
      write (77,*)

      write (77,*) "-------------------------"
      write (77,*) "units of thermostat mass"
      write (77,*) "-------------------------"
      write (77,*) "kB= 1.3806504 x 10^{-23}       Joule/Kelvin"
      write (77,*) "1 Joule= 2.29371248 x 10^{17} Hartree"
      write (77,*) "1 mole= 6.02214179 x 10^{23}"
      write (77,*)

      write (77,*) "Mt= (KJoule/Mole) x ps^2"
      write (77,*) "  = 1000 x (2.29371248 x 10^{17} Hartree) x",
     &            " (10^{-12} sec)^2 /6.02214179 x 10^{23}"
      write (77,*) "  = (2.29371248/ 6.02214179) x 10^(3+17-24-23)",
     &            " Hartree * sec^2"
      write (77,*) "  = 3.808798530 x 10^{-27} Hartree * sec^2"
      write (77,*)

      write (77,*) "fmt= 3.808798530d-27"
      write (77,*) "(KJoule/Mole) x ps^2= fmt * (Hartree x sec^2)"
      write (77,*)

      write (77,*) "-------------------------"
      write (77,*) "units of frequency"
      write (77,*) "-------------------------"
      write (77,*) "(N/cm)/amu= (Kg mt/s^2)/ (cm *1.6605402x10^{-27} 
     & Kg)"
      write (77,*) "          = (100 cm/s^2)/(cm *1.6605402x10^{-27})"
      write (77,*) "          = (1/s^2)/1.6605402x10^{-29}"
      write (77,*)

      write (77,*) "[freq]= sqrt ([spring K/mass]"
      write (77,*) "      = sqrt [(N/cm)/amu]"
      write (77,*) "      = 1/sqrt(1.6605402x10^{-29}) s^{-1}"
      write (77,*)

      write (77,*) "facFreq= 1./sqrt(1.6605402d-29)"
      write (77,*) "sqrt[(N/cm)/amu]= facFreq * s^{-1}"
      write (77,*)

      write (77,*) "-------------------------------------------"
      write (77,*) "units for speed introducing the universal constant",
     &            " of gases"
      write (77,*) "-------------------------------------------"
      write (77,*) "universal constant of gases   R=8.3144621 J/",
     &            " (mol*Kelvin)"
      write (77,*) "Joule= newton*m= Kg*m/s^2 *m= Kg*m^2/s^2"
      write (77,*) "mole= 6.02214179 *10^{23}"
      write (77,*)

      write (77,*) "amu= 1.6605402 x10^{-27} Kg= 1.6605402*10^{-27} Kg",
     &            " *6.02214179*10^{23}/ mol"
      write (77,*) "     = 1.6605402 *6.02214179 *10^{-4} Kg/mol"
      write (77,*) "     = 1*10^{-3} Kg/mol"
      write (77,*)

      write (77,*) "[sqrt(RT/mass)]= sqrt [J/(mol*Kelvin) * Kelvin/",
     &            " (1*10^{-3}*Kg/mol)]"
      write (77,*) "               = sqrt [10^3 (m/s)^2]"
      write (77,*) "               = sqrt [10^3] m/s"
      write (77,*)
     ...
     &            " Angst/fs"
      ...
      write (77,*) "fac= 10^{-5}* sqrt(1000.)"
      write (77,*)

      write (77,*) "[sqrt(RT/m)]= sqrt [J/(mol*Kelvin) * Kelvin/amu]=",
     &            " fac *Angst/fs"
      write (77,*)

      write (77,*) "-----------------------------------"
      write (77,*) "convert energy to wavelength units"
      write (77,*) "-----------------------------------"
      write (77,*) "energy= hbar*freq"
      write (77,*) "lambda*freq= c"
      write (77,*)

      write (77,*) "energy= hbar*c/lambda"
      write (77,*) "lambda= hbar*c/energy"
      write (77,*)

      write (77,*) "hbar= 6.62606957 x10^{-34} (kg*m^2)/s"
      write (77,*) "c= 2.99792458 x10^8 m/s"
      write (77,*) "1 Hartree= 4.35988 x10^{-18} J=N*m=(kg*m/s^2)*m=",
     &            " kg*m^2/s^2"
      write (77,*) "alpha [hartree]= 4.35988 x10^{-18} *alpha [J]"
      write (77,*)

      write (77,*) "lambda= c*hbar/energy [(m/s)*((kg*m^2)/s) /J]= mt"
      write (77,*) "      = (2.99792458* 6.62606957/4.35988)",
     &            " x10^{8-34+18}/alpha  mt"
      write (77,*) "      = [(4.55619348)/alfa] x10^{-8} mt"
      write (77,*)

      write (77,*) "1 mt= 1x10^{+10} Angst"
      write (77,*) "lambda= [(455.619348)/alfa]  Angst"
      write (77,*)

      write (77,*) "calculator: frequency-wavelength-energy"
      write (77,*) "http://www2.chemistry.msu.edu/faculty/reusch",
     &            "/virttxtjml/cnvcalc.htm"
      write (77,*)

      write (77,*) "ionizing & non-ionizing radiation information"
      write (77,*) "http://www.epa.gov/radiation/understand",
...
c     include 'parametros'
      ...
      dimension  x(nmax), y(nmax), z(nmax)
        ...
        CHARACTER (LEN = 400) RutaArchivosEntrada
        CHARACTER (LEN = 400) RutaArchivosSalida
        CHARACTER (LEN = 400) MolXYZ 

        call DatosGrlsDinamica(NombreDinamica, 
     & RutaArchivosEntrada, RutaArchivosSalida)
        
        MolXYZ = trim(RutaArchivosSalida)
     & // '/MolXYZ_' // trim(NombreDinamica)
     & // '.out'

      open (unit=21, file=trim(MolXYZ), status= 'unknown')

      write(21,*) npar
      write(21,*) "molec"

      do m= 1, npar
      write(21,35) atsymbol(m), x(m),y(m),z(m)
...
* ***********************************
     ...
     & RutaPermanent, BaseGrlNwchem) 
        ...
        INTEGER NumeroCarpetas, i
        CHARACTER (LEN = 400) Carpeta
     ...
     & // '.dat'
        ...
        READ(9, *) NumeroCarpetas
        !inicio do carpetas 
        DO i = 1, NumeroCarpetas
        RutaScratch = trim(RutaScratch) // "/"
        READ(9,*) Carpeta 
        RutaScratch = trim(RutaScratch)//trim(Carpeta)
        print*, trim(RutaScratch)
        END DO 
        !termina do carpetas
        ...
        !inicio do carpetas 
        DO i = 1, NumeroCarpetas
        RutaPermanent = trim(RutaPermanent) // "/"
        READ(9,*) Carpeta 
        RutaPermanent = trim(RutaPermanent) // Carpeta
        END DO 
        !termina do carpetas
        ...
        subroutine RutinaDatosNwchem2(RutaMPI, 
     & RutaNWChem, NumeroProcesadores) 
    
        CHARACTER (LEN = 400) NombreDinamica
        CHARACTER (LEN = 400) RutaArchivosEntrada
        CHARACTER (LEN = 400) RutaArchivosSalida
        CHARACTER (LEN = 400) DatosNwchem2

        CHARACTER (LEN = 400) RutaMPI
        CHARACTER (LEN = 400) RutaNWChem
        CHARACTER (LEN = 400) NombreCPU
        INTEGER NumeroCarpetas, i, NumeroProcesadores
        CHARACTER (LEN = 400) Carpeta
     ...
     & RutaArchivosEntrada, RutaArchivosSalida)
        ...
        DatosNwchem2 = trim(RutaArchivosEntrada)
     & // '/DatosNwchem2_' // trim(NombreDinamica)
     & // '.dat'
  
        OPEN(9, FILE = trim(DatosNwchem2))
        ...
        !inicio do carpetas 
        DO i = 1, NumeroCarpetas
        RutaMPI = trim(RutaMPI) // "/"
        READ(9,*) Carpeta 
        RutaMPI = trim(RutaMPI) // Carpeta
        END DO 
        !termina do carpetas
        
        READ(9,*) NumeroProcesadores


        READ(9, *) NumeroCarpetas
        !inicio do carpetas 
        DO i = 1, NumeroCarpetas
        RutaNWChem = trim(RutaNWChem) // "/"
        READ(9,*) Carpeta 
        RutaNWChem = trim(RutaNWChem) // Carpeta
        END DO
        !termina do carpetas
        ...
        end subroutine RutinaDatosNwchem2
